...
function normalizeAvgText(value: string | undefined) {
  const n = parseFloat(String(value ?? "").trim());
  return Number.isFinite(n) ? formatRate(n) : ".000";
}

function parseTableRows($: cheerio.CheerioAPI, tableIndex: number): string[][] {
  const rows: string[][] = [];
  $("table")
    .eq(tableIndex)
    .find("tr")
...
function parseSituationRows(
  $: cheerio.CheerioAPI,
  tableIndex: number
): SituationSplit[] {
  return parseTableRows($, tableIndex)
    .filter(cols => cols.length >= 11 && cols[0])
    .map(cols => ({
      label: cols[0] ?? "",
      avg: normalizeAvgText(cols[1]),
      ab: parseNumberText(cols[2]),
      hits: parseNumberText(cols[3]),
      doubles: parseNumberText(cols[4]),
      triples: parseNumberText(cols[5]),
      hr: parseNumberText(cols[6]),
      rbi: parseNumberText(cols[7]),
      bb: parseNumberText(cols[8]),
      hbp: parseNumberText(cols[9]),
      so: parseNumberText(cols[10]),
      gdp: parseNumberText(cols[11]),
    }));
}
...
function seasonAvgFromSplits(splits: SituationSplit[]) {
  ...
  return ab > 0 ? formatRate(hits / ab) : ".000";
}
...
export async function getRecentGames(
  ...
  for (const teamShort of teams) {
    const last10 = allGames
      .filter(
        g =>
          getTeamInfo(g.away).short === teamShort ||
          getTeamInfo(g.home).short === teamShort
      )
      .slice(-10);
...
export async function getHittersCombined(season = "2026", page = 1) {
  ...
  const data = basic1.data.map((p: any) => {
    ...
    const babip =
      babipDenom > 0 ? ((p.hits - p.hr) / babipDenom).toFixed(3) : "0.000";
...
// ─── 타자 상황별 기록 (볼카운트, 투수유형) ───────────────
export async function getHitterSituation(playerId: string) {
  const cacheKey = `hitter_situation_${playerId}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;
  ...
  const counts = parseSituationRows($, 1);
  const vsTypes = parseSituationRows($, 4);
  const result = {
    playerId,
