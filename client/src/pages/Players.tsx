  ...
  return (
    <Link href={getPlayerDetailPath(player)}>
      <div className="flex cursor-pointer items-center gap-2 border-b border-border px-4 py-2.5 transition-colors hover:bg-accent">
        ...
        <div
          className="grid flex-1 gap-1 text-right"
          style={{
        ...
        >
          {visibleCols.map((col, ci) => {
            ...
            const isSaber = isHitter
              ? SABER_HITTER_KEYS.has(col.key)
              : SABER_PITCHER_KEYS.has(col.key);
            const isMain = ci === 0;
            return (
              <div key={col.key} title={col.title}>
                <div
                  className={`font-stat text-xs font-semibold ${isMain ? "font-black text-primary" : isSaber ? "text-note" : "text-foreground"}`}
                >
                  {val}
                </div>
              </div>
    ...
    <div className="min-h-[calc(100vh-65px)] bg-background text-foreground">
      <div className="mx-auto w-full max-w-[1440px] px-4 py-7 sm:px-6 lg:px-8">
        ...
        <section className="mb-5 flex flex-wrap items-center gap-3 rounded-[6px] border border-border bg-muted px-4 py-3 shadow-[0_1px_2px_rgb(17_24_39/0.08)] sm:px-5">
          <Tabs
            value={tab}
            onValueChange={v => setTab(v as "hitter" | "pitcher")}
          >
          ...
          >
            {tab === "hitter" ? "규정타석" : "규정이닝"}{" "}
            {qualifiedOnly ? "ON" : "OFF"}
          </button>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: `${tableMinWidth}px` }}>
              ...
              <div className="flex items-center gap-2 border-b border-border-strong bg-muted px-4 py-2.5">
                <div className="w-7 text-xs font-black uppercase text-muted-foreground">
                  #
                </div>
                <div className="w-32 text-xs font-black uppercase text-muted-foreground">
                  선수
                </div>
                <div
                  className="grid flex-1 gap-1 text-right"
              ...
              {loading ? (
                ...
                <div>
                  {sorted.map((player, i) => (
                    <PlayerRow
                      key={`${player.playerId || player.playerName}-${i}`}
                      player={player}
                      type={tab}
                      index={i}
