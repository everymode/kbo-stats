...
## 서버 통합 (배포 준비)
- [x] Node.js Express 서버에 KBO 크롤링 API 구현 (server/kbo.ts)
- [x] Express REST API로 KBO 엔드포인트 구현 (server/index.ts)
- [x] 프론트엔드 API 클라이언트를 Node.js 서버 경로로 변경
- [x] 통합 테스트 (팀 순위, 타자, 투수, 리더보드) 완료
- [x] 최종 체크폰인트 저장

## 버그 수정 및 기능 개선
- [x] 다크/라이트 모드 토글 버그 수정 (CSS 변수 구조 수정)
- [x] 선수 목록 페이지 홈런/타점/안타 데이터 표시 수정 (Basic1+Basic2 통합)
- [x] 세이버메트릭스 계산 추가 (BB%, K%, ISO, BABIP, FIP, K/9, BB/9, HR/9)
- [x] 타자 스탯에 OPS, OBP, SLG, BB%, K%, ISO, BABIP 추가
- [x] 투수 스탯에 FIP, K/9, BB/9, HR/9 추가
- [x] 리더보드에 세이버메트릭스 카테고리 탭 추가
- [x] 선수 프로필에 세이버메트릭스 스탯 카드 추가
